import React from 'react';
import { useNavigate } from 'react-router-dom';

const Header: React.FC = () => {
  const navigate = useNavigate();

  const handleAuthClick = () => {
    navigate('/auth');
  };

  return (
    <header className="sticky top-0 z-100 flex justify-between items-center px-4 sm:px-6 md:px-8 py-3 sm:py-4 bg-[#17151E] border-b border-[#1e1f22]">
      <div className="flex items-center gap-2 sm:gap-4 md:gap-6">
        <div className="flex items-center gap-2 text-lg sm:text-xl font-semibold text-white cursor-pointer" onClick={() => navigate('/')}>
          <img src="/src/assets/fire-logo.png" alt="Okibi Logo" className="w-4 h-6 sm:w-5 sm:h-7" />
          <span>Agentmint</span>
        </div>
        <button className="hidden md:block px-5 py-1 bg-[#33201E] text-white border-[0.1px] border-gray-600 rounded-xl text-base font-normal hover:border-amber-600 transition-colors">
          Request a Custom Agent
        </button>
      </div>
      <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
        <a href="#" className="hidden sm:flex items-center justify-center w-10 h-10 rounded-xl text-[#b5bac1] bg-[#26242E] hover:text-white transition-all">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
            <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515a.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0a12.64 12.64 0 0 0-.617-1.25a.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057a19.9 19.9 0 0 0 5.993 3.03a.078.078 0 0 0 .084-.028a14.09 14.09 0 0 0 1.226-1.994a.076.076 0 0 0-.041-.106a13.107 13.107 0 0 1-1.872-.892a.077.077 0 0 1-.008-.128a10.2 10.2 0 0 0 .372-.292a.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127a12.299 12.299 0 0 1-1.873.892a.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028a19.839 19.839 0 0 0 6.002-3.03a.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419c0-1.333.956-2.419 2.157-2.419c1.21 0 2.176 1.096 2.157 2.42c0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419c0-1.333.955-2.419 2.157-2.419c1.21 0 2.176 1.096 2.157 2.42c0 1.333-.946 2.418-2.157 2.418z" />
          </svg>
        </a>
        <a href="#" className="hidden sm:flex items-center justify-center w-10 h-10 rounded-xl text-[#b5bac1] bg-[#26242E] hover:text-white transition-all">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
            <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
          </svg>
        </a>
        <button onClick={handleAuthClick} className="px-3 sm:px-4 py-1 bg-white text-[#2b2d31] rounded-md text-xs sm:text-sm font-sm hover:bg-[#e8e8e8] transition-colors">
          Sign In
        </button>
        <button onClick={handleAuthClick} className="hidden sm:block px-4 py-1 text-white border border-[#4e5058] rounded-md text-sm font-medium bg-[#35373c] transition-colors">
          Sign Up
        </button>
      </div>
    </header>
  );
};

export default Header;
